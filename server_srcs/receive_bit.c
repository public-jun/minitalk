...
/*   Updated: 2021/06/27 15:59:28 by jnakahod         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						 ...
						 char (*p_client)[], t_bool_info *info)
{
	...
	{
		pause();
		if (g_t_data.now_bit == 8)
		{
			if (info->b_client_pid == false && !(*p_client)[g_t_data.now_byte])
				client_pid = get_client_pid(info, p_client, mess);
			else if ((*mess)[g_t_data.now_byte] == EOT)
			{
				print_mess(mess, client_pid);
				reset_buff(mess, p_client, &client_pid, info);
			}
			else if (g_t_data.now_byte + 1 == BUFFER_SIZE)
